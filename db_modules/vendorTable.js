...
module.exports =
{
    ...
    getconstdetails: function (check) {


        return db.constants.find(check).lean().exec();
    ...
    getSalonBookings: function (salonId, employeeId, date, languagesCode, callback) {
        ...
        db.stylist.aggregate([
            ...
            isabroad = 0
            var vendordetails = await db.stylist.find({ vendor_id: vendorId }, { nationality: 1, country: 1 }).lean().exec();
            var countrydetails = await db.country.find({ _id: vendordetails[0].country }, { country_code: 1, country: 1 }).lean().exec();
            if ((vendordetails[0].nationality == countrydetails[0].country['en']) || (vendordetails[0].nationality == countrydetails[0].country['tr'])) {
                isabroad = 0;
            } else {
                isabroad = 1;
            }

            return callback(response, isabroad);
        });



    }, getCountryabroadDocuments: function (vendorId, languageCode, callback) {
        var vendor = mongoose.Types.ObjectId(vendorId);
        db.stylist.aggregate([
            { "$match": { "vendor_id": vendor } },
            {
                "$lookup": {
                    ...
                    "pipeline": [
                        { "$match": { "$expr": { "$and": [{ "$eq": ["$_id", "$$country"] }] } } },
                        { "$unwind": "$stylist_abroad_documents" },
                        {
                            "$lookup": {
                                "from": "documents", "let": { "document_id": "$stylist_abroad_documents" },
                                "pipeline": [{ "$match": { "$expr": { "$and": [{ "$eq": ["$_id", "$$document_id"] }, { "$eq": ["$status", 1] }] } } }]
                                , "as": "documents"
                            }
                        },
    ...
    updateReferAmountWithPromises: function (data, where) {
        ...
        if (req.body.status == "success") {
            console.log("comming to table if")
            db.vendorbankdetails.update({ _id: req.body.id }, { $set: { submarchantkey: req.body.submarchantkey, status: true, iban_result: req.body.iban_result } }).lean().exec((err, data) => {
            ...
            console.log("comming to table else")
            db.vendorbankdetails.update({ _id: req.body.id }, { $set: { iban_result: req.body.iban_result } }).lean().exec((err, data) => {
                callback(err)
        ...
        if (req.body.status == "success") {
            db.vendorbankdetails.update({ _id: req.body.id }, { $set: { setIban: req.body.setIban, submarchantkey: req.body.submarchantkey, status: true, ibanname: req.body.ibanname, taxNumber: req.body.taxNumber, iban_result: req.body.iban_result } }).lean().exec((err, data) => {
                callback(err, data)
            })
        } else {
