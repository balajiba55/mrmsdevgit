...
router.post('/check-mobile', tokenValidations, async function (req, res) {
    ...
    if (otpType == utility.OTP_TYPE_CUSTOMER_SIGN_UP) {
        ...
        async.series([
            ...
            function (data) {
                ...
                tables.vendorTable.update({ "status": 2 }, { _id: userId, otp: otp }, function (result) {
        ...
        if (encryptPassword != undefined && encryptPassword != '') {
            if (decrypt(encryptPassword, hash) == password) {
                ...
                if (result[0].type == 1 || result[0].type == 3) {
                    ...
                    var accessToken = '';
                    tables.salonTable.find({ "vendor_id": result[0]._id }, function (response) {
                        var fcmData = {};
                        fcmData['fcm_id'] = fcmId;
                        fcmData["device_id"] = deviceId;
                        deviceType = parseInt(deviceType);
...
router.post('/expertise', tokenValidations, function (req, res) {
    var languageCode = req.body.language_code;
    var type = req.body.type;
...
router.post('/check-country', tokenValidations, async function (req, res) {
    ...
    condition["status"] = 1;
    tables.countryTable.findCountry(condition, { "_id": 1, "country": { "$ifNull": ["$country." + languageCode, '$country.en'] }, "phone_code": 1 }, function (response) {
...
router.post('/get-cities', tokenValidations, function (req, res) {
    var country = req.body.country;
    country = country.toLowerCase();
    //res.send(country);
...
router.post('/add-payment-deteils', async function (req, res) {
    console.log("add-payment-deteils", req.body)
    ...
    tables.vendorTable.addvendorbankdetails(obj, (err, data) => {
        if (data == "already added") {
            ...
            return res.send({
                "success": true,
                "status": 1,
                "url": `http://mrmsbeauty.com/payment_sdk/samples/create_sub_merchant.php?str=${encriptobj}`,
                "data": data
...
router.post('/update-iban', async function (req, res) {
    ...
    await tables.vendorTable.getiban(req, (err, getvendoribandetails) => {
        ...
        var encriptobj = Buffer.from(finalobj).toString('base64')
        console.log("http://mrmsbeauty.com/payment_sdk/samples/update_sub_merchant.php?str=${encriptobj}", `http://mrmsbeauty.com/payment_sdk/samples/update_sub_merchant.php?str=${encriptobj}`)
        return res.send({
            "success": true,
            "status": 1,
            "url": `http://mrmsbeauty.com/payment_sdk/samples/update_sub_merchant.php?str=${encriptobj}`
...
router.post('/vendor-online-payment', async function (req, res) {
    ...
    req.body.status = "0"
    req.body.paidPrice = ((Number(req.body.price) + Number(utility.iyzico_commission)) + ((Number(req.body.price) * Number(utility.iyzico_percentage)) / 100)).toFixed(2)
    console.log("req.body.paidPrice>>>>>>>>>>", req.body.paidPrice)
    tables.paymentcardTable.vendoraddpayment(req.body, (response) => {
        ...
        if (response != undefined) {
            ...
            var encriptobj = Buffer.from(finalobj).toString('base64')
            console.log("`http://mrmsbeauty.com/payment_sdk/samples/create_payment_with_registered_card.php?str=${encriptobj}", `http://mrmsbeauty.com/payment_sdk/samples/create_payment_with_registered_card.php?str=${encriptobj}`)
            return res.send({
                "success": true,
                "status": 1,
                "url": `http://mrmsbeauty.com/payment_sdk/samples/create_payment_for_vendor.php?str=${encriptobj}`
